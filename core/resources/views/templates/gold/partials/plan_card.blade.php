        ...
        <ul class="custom--tab nav nav-pills" id="pills-tab" role="tablist">
            @foreach ($miners as $item)
                <li class="nav-item" role="presentation">
                    <button class="nav-link fs-16 @if ($loop->first) active @endif"
                        id="pills-{{ $loop->iteration }}-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-{{ $loop->iteration }}" type="button" role="tab"
                        aria-controls="pills-{{ $loop->iteration }}"
                        aria-selected="@if ($loop->first) true @else false @endif"><span
                            class="text">{{ $item->name }}</span></button>
                </li>
            @endforeach
        </ul>
    @endif
    <div class="tab-content" id="pills-tabContent">
        @foreach ($miners as $item)
            <div class="tab-pane fade @if ($loop->first) show active @endif"
                id="pills-{{ $loop->iteration }}" role="tabpanel" aria-labelledby="pills-{{ $loop->iteration }}-tab"
                tabindex="0">
                            ...
                            <div class="pricing-item">
                                <div class="pricing-item__content">
                                    ...
                                    <div class="pricing-item__header">
                                        ...
                                        <h3 class="pricing-item__price mb-0"> <span
                                                class="text--gradient d-flex align-items-start"> <small
                                                    class="dollar">{{ $general->cur_sym }}</small><span
                                                    class="price">{{ showAmount($plan->price) }}</span> </span> <small
                                                class="text">
                                    ...
                                    <div class="pricing-item__body">
                                        <ul class="text-list">
                                            ...
                                            <li class="text-list__item fs-18">{{ getAmount($plan->maintenance_cost) }}%
                                                @lang('Maintenance cost per day')</li>
